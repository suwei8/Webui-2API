...
将 Google Gemini / ChatGPT Web 转换为支持 OpenAI 格式的本地 API 服务。

本项目通过自动化控制 Chromium 浏览器与 `gemini.google.com` 或 `chatgpt.com` 交互，对外提供一个兼容 OpenAI `/v1/chat/completions` 的 HTTP 接口。

## 功能特性

- **OpenAI 兼容**: 支持标准的 `/v1/chat/completions` 接口调用。
- **多模型支持**: 支持 `gemini` 和 `chatgpt` 模型。
- **本地运行**: 数据直接在本地浏览器与 Google/OpenAI 服务器交互。
...
- **账号**: 必须在 Chromium 中预先登录 Google Gemini 和/或 ChatGPT。
...
2. **配置说明**
   本项目设计为使用 Snap 版 Chromium。如果您的浏览器路径不同，请修改 `src/browser.js` 和 `src/browser_chatgpt.js` 中的 `CHROMIUM_PATH` 和 `USER_DATA_DIR`。
...
- **ChatGPT Selector**: ChatGPT 网页结构更新频繁，如遇到无法输入或提取响应的情况，可能需要更新 `src/chatgpt.js` 中的 `SELECTORS`。

## 目录结构

- `src/server.js`: API 服务器入口。
- `src/browser.js`: Gemini 浏览器自动化控制逻辑。
- `src/browser_chatgpt.js`: ChatGPT 浏览器自动化控制逻辑。
- `src/gemini.js`: Gemini 网页交互核心逻辑。
- `src/chatgpt.js`: ChatGPT 网页交互核心逻辑。
